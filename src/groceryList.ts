import { Stock, Ingredient } from "./types.js";
import { UnitType, toBaseUnit, getUnitDimension, BASE_UNITS, formatQuantity } from "./units.js";
...
export function saveGroceryList(list: GroceryItem[]): void {
...
export async function addRecipeToGroceryList(ingredients: Ingredient[]): Promise<{
  added: GroceryItem[];
  skipped: string[];
}> {
...
function addItemToGroceryList(list: GroceryItem[], item: GroceryItem): void {
...
export function removeFromGroceryList(index: number): void {
...
export function toggleGroceryItemChecked(index: number): void {
  const list = getGroceryList();
  if (index >= 0 && index < list.length) {
...
export function updateGroceryItemQuantity(index: number, newQuantity: number): void {
  const list = getGroceryList();
...
export function getCheckedItems(): GroceryItem[] {
