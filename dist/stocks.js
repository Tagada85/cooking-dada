...
function addSingleIngredientToStock(ingredient, actualStocks) {
    const stockIndex = actualStocks.findIndex((stock) => stock.name.toLowerCase() === ingredient.name.toLowerCase());
...
export async function addMultipleToStock(ingredients) {
    const actualStocks = await loadStocks();
    const errors = [];
...
export async function cookRecipe(recipe) {
    const actualStocks = await loadStocks();
    const deducted = [];
    const missing = [];
